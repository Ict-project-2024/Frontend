...
const AdminDashboard = ({ userId, userName }) => {
	...
	useEffect(() => {
		...
		newApiRequest(`${import.meta.env.VITE_BASE_URL}:${import.meta.env.VITE_PORT}/api/library/history`, 'GET', {})
			.then(response => {
				if (response.success) {
	...
	// Fetch the required data for library: nivindulakshitha
	useEffect(() => {
		newApiRequest(`${import.meta.env.VITE_BASE_URL}:${import.meta.env.VITE_PORT}/api/library/useraccess`, 'POST', {}) // Time slot should be included
			.then(response => {
				if (response.success) {
					// Set the data for library: nivindulakshitha
					let index = 0;
					response.data.map((user) => {
						...
						newApiRequest(`${import.meta.env.VITE_BASE_URL}:${import.meta.env.VITE_PORT}/api/user/`, 'POST', { "teNumber": user.teNumber })
							.then(result => {
	...
	useEffect(() => {
		newApiRequest(`${import.meta.env.VITE_BASE_URL}:${import.meta.env.VITE_PORT}/api/medical-center/useraccess`, 'POST', {}) // Time slot should be included
			.then(response => {
				if (response.success) {
	...
	useEffect(() => {
		...
		newApiRequest(`${import.meta.env.VITE_BASE_URL}:${import.meta.env.VITE_PORT}/api/medical-center/history`, 'GET', {})
			.then(response => {
				if (response.success) {
					// Set the data for library: nivindulakshitha
					response.data.map((data, index) => {
